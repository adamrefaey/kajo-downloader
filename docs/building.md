...
**FFmpeg (Linux/Windows)** comes from [BtbN/FFmpeg-Builds](https://github.com/BtbN/FFmpeg-Builds). Upstream keeps only ~14 daily autobuilds, but **month-end** builds for ~2 years — we pin a month-end `autobuild-*` tag plus **n8.1 release-line** GPL assets (not master `N-*`, which disappear within weeks). macOS still uses `eugeneware/ffmpeg-static`.

Before a full download, CI runs [`scripts/verify-binary-pins.mjs`](../scripts/verify-binary-pins.mjs) (`pnpm run binaries:verify`) to confirm each release tag still exists and lists the pinned asset names.

To refresh FFmpeg pins (default: newest month-end with n8.1 GPL assets):

```bash
pnpm run binaries:bump-ffmpeg
# or: pnpm run binaries:bump-ffmpeg -- --tag autobuild-2026-06-30-13-34
```
