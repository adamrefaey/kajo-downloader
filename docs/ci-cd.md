...
- **Dependabot is disabled** — no `.github/dependabot.yml`; dependency and action bumps are manual.
...
| Step | Command / action |
| ---- | ---------------- |
| Verify binary pin liveness | `pnpm run binaries:verify` — GitHub release tags + asset names exist (no download) |
