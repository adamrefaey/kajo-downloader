...
function validateBinaryPins(pins) {
    ...
    const tag = pins?.ytdlp?.tag;
    if (typeof minCal !== 'string' || typeof tag !== 'string') {
        throw new Error('binary-pins.json: missing minimumYtDlpCalver or ytdlp.tag');
    }
    if (compareCalver(tag, minCal) < 0) {
        throw new Error(`yt-dlp pin ${tag} is below minimumYtDlpCalver ${minCal}`);
    }
}
...
async function fetchYtDlp({ pins, platform, arch, outputDir, tempDir }) {
    ...
    if (platform === 'darwin') {
        ...
        const release = await getGitHubReleaseByTag(y.owner, y.repo, y.tag);
...
async function fetchFfmpegBundle({ pins, platform, arch, outputDir, tempDir, targetId }) {
    ...
    const release = await getGitHubReleaseByTag(b.owner, b.repo, b.tag);
...
async function fetchDarwinFfmpegBundle({ pins, arch, outputDir }) {
    ...
    const release = await getGitHubReleaseByTag(d.owner, d.repo, d.tag);
...
async function getGitHubReleaseByTag(owner, repo, tag) {
