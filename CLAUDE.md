...
| Binary fetch | `pnpm run dev` runs `scripts/ensure-dev-binaries.mjs`, which fetches pinned yt-dlp / ffmpeg / ffprobe / Deno (yt-dlp's JS runtime) when missing. Set `KAJO_SKIP_DEV_BINARIES=1` to skip. BtbN ffmpeg pins use month-end autobuild tags + n8.1 GPL assets (`pnpm run binaries:verify` / `binaries:bump-ffmpeg`). |
